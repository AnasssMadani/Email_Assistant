...
/** The specific headers packages/core's free pre-filter reads — see that package's prefilter.ts. Kept small deliberately, not a full raw-header dump. */
const PREFILTER_HEADER_NAMES = [
  "list-unsubscribe",
  "list-id",
  "list-post",
  "precedence",
...
export function extractPrefilterHeaders(
  headers: gmail_v1.Schema$MessagePartHeader[] | undefined
): Record<string, string | undefined> {
  const out: Record<string, string | undefined> = {};
  for (const name of PREFILTER_HEADER_NAMES) {
    out[name] = getHeader(headers, name);
  }
  return out;
}
